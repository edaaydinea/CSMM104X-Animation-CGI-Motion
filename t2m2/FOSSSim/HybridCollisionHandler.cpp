...
{	
    // Your code goes here!
    int i = 0;
    VectorXs new_position;
    VectorXs new_velocity;
    ...
    while(i < m_maxiters )
    {
        ...
        {
            new_position = qefinal;
            new_velocity = qdotefinal;
            applyImpulses(scene, collisions_found, qs, new_position, new_velocity, dt, qefinal, qdotefinal);
        }
...
void HybridCollisionHandler::performFailsafe(const TwoDScene &scene, const VectorXs &qs, const ImpactZone &zone, double dt, VectorXs &qe, VectorXs &qdote)
{
    ...
    VectorXs dx;
    VectorXs center_of_mass(2);
    center_of_mass.setZero();
    VectorXs p(2);
    p.setZero();
    
    double mass = 0;
    double total_angular_momentum = 0;
    double inertia = 0;
    bool handleFixedObject = false;
    ...
    for(std::set<int>::iterator i = zone.m_verts.begin(); i != zone.m_verts.end(); ++i)
    {
        ...
        center_of_mass = center_of_mass + mi * xi;
        p = p + mi * dxi;
        mass = mass + mi;
        
        if (scene.isFixed(*i))
            handleFixedObject = true;
    }
    ...
    for(std::set<int>::iterator i = zone.m_verts.begin(); i != zone.m_verts.end(); ++i)
    {
        VectorXs velocity_i = dx.segment<2>(2* *i) - p;
        VectorXs rigid_i = qs.segment<2>(2* *i) - center_of_mass;
        scalar mi = scene.getM()[2* *i];
        
        total_angular_momentum = rigid_i[0]*velocity_i[1] - rigid_i[1]*velocity_i[0];
        total_angular_momentum = total_angular_momentum * mi;
        total_angular_momentum = total_angular_momentum + total_angular_momentum;
        
        inertia = inertia + mi * rigid_i.dot(rigid_i);
         
    }
    
    if(handleFixedObject || zone.m_halfplane)
    {
        for(std::set<int>::iterator i = zone.m_verts.begin(); i != zone.m_verts.end(); ++i)
        {
            qe.segment<2>(2* *i) = qs.segment<2>(2* *i);
            qdote.segment<2>(2* *i) = qdote.segment<2>(2* *i) * 0;
        }
        return;
    }
    
    double omega  = total_angular_momentum / inertia;
    
    for(std::set<int>::iterator i = zone.m_verts.begin(); i != zone.m_verts.end(); ++i)
    {
        VectorXs rigid_i = qs.segment<2>(2* *i) - center_of_mass;
        VectorXs rigid_i_normal = rigid_i;

        std::swap(rigid_i_normal[0], rigid_i_normal[1]);
        rigid_i_normal[0] = -1.0 * rigid_i_normal[0];
        
        qe.segment<2>(2* *i) = center_of_mass + p + cos(omega)*rigid_i + sin(omega)*rigid_i_normal;
